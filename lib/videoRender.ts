...
export async function renderReel(opts: RenderOptions): Promise<RenderResult> {
  ...
  const drawLoop = () => {
    ...
    if (phase === "main") {
      ...
      // Corner logo intentionally omitted — logo only appears in the outro end card.
  ...
  // Pre-seek to first segment so the recorder's very first frame is the right shot.
...
/* drawLogo / drawBrandText kept for future use even though we no longer call
   them in the main phase — leaving the helpers makes it trivial to re-enable
   a corner watermark via a new RenderOption later. */
...
void drawLogo; void drawBrandText;
