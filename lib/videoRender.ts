...
/* Browser-side reel renderer with cinematic finish:
   ...
   - MediaRecorder tries native MP4 first (Chrome 119+, Edge 119+, Safari) so most
     users skip ffmpeg.wasm transcoding entirely. */
...
export async function renderReel(opts: RenderOptions): Promise<RenderResult> {
  ...
  // When MP4 wins here, the ReelMultiplier render flow skips ffmpeg.wasm transcoding entirely.
  const candidates = [
