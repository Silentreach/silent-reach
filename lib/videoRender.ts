...
export async function renderReel(opts: RenderOptions): Promise<RenderResult> {
  ...
  // Try MP4 first — Chrome 119+ / Edge 119+ / Safari support it natively.
  const candidates = [
  ...
  // Pre-seek so the recorder's very first frame is the right shot.
...
function drawAnimatedHook(
  ...
  if (bgStyle !== "none") {
    ...
    // Soft vertical gradient band so the type is readable on any background.
